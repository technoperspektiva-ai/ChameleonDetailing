...
export async function buildGoogleSheetsSnapshot(env:SheetsEnv,trigger='manual'):Promise<SheetsPayload>{
 ...
 const [users,orders,payments,revenue,vip,referrals,analytics,audit,cars,packages,rewards,reviews,suggestions]=await Promise.all([
  ...
  query(env,`SELECT r.id,r.user_id,COALESCE(u.first_name,'') customer,COALESCE(u.username,'') username,r.status,r.request_type,COALESCE(r.service_slug,'') service,COALESCE(r.services_json,'[]') services_json,COALESCE(r.options_json,'[]') options_json,COALESCE(r.car_id,'') car_id,COALESCE(r.car_name,'') car_name,COALESCE(r.car_plate,'') car_plate,COALESCE(r.vehicle_slug,'') vehicle,COALESCE(r.condition_slug,'') condition,COALESCE(r.calculated_price,0) calculated_price,COALESCE(r.final_job_price,'') final_job_price,r.currency,COALESCE(r.emergency_multiplier,'') emergency_multiplier,COALESCE(r.emergency_surcharge,0) emergency_surcharge,r.payment_status,r.created_at,COALESCE(r.confirmed_at,'') confirmed_at,COALESCE(r.completed_at,'') completed_at,COALESCE(r.client_deleted_at,'') client_deleted_at,COALESCE(r.staff_deleted_at,'') staff_deleted_at FROM service_requests r LEFT JOIN users u ON u.id=r.user_id WHERE COALESCE(r.is_test,0)=0 ORDER BY r.id DESC LIMIT 10000`),
  ...
  query(env,`SELECT c.id,c.user_id,COALESCE(u.first_name,'') customer,COALESCE(u.username,'') username,c.name,c.brand,c.model,c.modification,c.body_type,c.plate,c.has_ceramic,c.owner_phone,c.created_at,c.updated_at,(SELECT MAX(completed_at) FROM service_requests r WHERE r.car_id=c.id AND r.status='COMPLETED' AND r.staff_deleted_at IS NULL) last_service_at,(SELECT COUNT(*) FROM service_requests r WHERE r.car_id=c.id AND r.status='COMPLETED' AND r.staff_deleted_at IS NULL) visits FROM client_cars c LEFT JOIN users u ON u.id=c.user_id ORDER BY c.id DESC LIMIT 10000`),
  query(env,`SELECT p.car_id,c.user_id,COALESCE(u.first_name,'') customer,COALESCE(u.username,'') username,c.name car_name,c.plate,p.main_services_json,p.options_json,p.vehicle_slug,p.condition_slug,p.created_at,p.updated_at FROM car_packages p JOIN client_cars c ON c.id=p.car_id LEFT JOIN users u ON u.id=c.user_id ORDER BY p.updated_at DESC LIMIT 10000`),
  query(env,`SELECT rr.id,rr.referral_id,rr.user_id,COALESCE(u.first_name,'') user,COALESCE(u.username,'') username,rr.beneficiary,rr.reward_type,COALESCE(rr.reward_value,'') reward_value,COALESCE(rr.reward_currency,'') reward_currency,COALESCE(s.slug,'') reward_service,rr.status,rr.created_at,COALESCE(rr.used_at,'') used_at,COALESCE(rr.used_request_id,'') used_request_id FROM referral_rewards rr LEFT JOIN users u ON u.id=rr.user_id LEFT JOIN services s ON s.id=rr.reward_service_id ORDER BY rr.id DESC LIMIT 10000`),
  query(env,`SELECT f.id,f.user_id,COALESCE(u.first_name,'') user,COALESCE(u.username,'') username,f.text,CASE WHEN COALESCE(f.photo_file_id,'')<>'' THEN 'YES' ELSE 'NO' END photo_attached,COALESCE(f.photo_file_id,'') photo_file_id,f.language,f.status,f.created_at FROM client_feedback f LEFT JOIN users u ON u.id=f.user_id WHERE f.kind='REVIEW' ORDER BY f.id DESC LIMIT 10000`),
  query(env,`SELECT f.id,f.user_id,COALESCE(u.first_name,'') user,COALESCE(u.username,'') username,f.text,CASE WHEN COALESCE(f.photo_file_id,'')<>'' THEN 'YES' ELSE 'NO' END photo_attached,COALESCE(f.photo_file_id,'') photo_file_id,f.language,f.status,f.created_at FROM client_feedback f LEFT JOIN users u ON u.id=f.user_id WHERE f.kind='SUGGESTION' ORDER BY f.id DESC LIMIT 10000`),
 ]);
 const [totalUsers,clients,requestCount,completedCount,paidCount,vipCount,opens30,revenueTotal,carCount,packageCount,rewardCount]=await Promise.all([
  ...
  scalar(env,'SELECT COUNT(*) n FROM client_cars'),
  scalar(env,'SELECT COUNT(*) n FROM car_packages'),
  scalar(env,"SELECT COUNT(*) n FROM referral_rewards WHERE status='AVAILABLE'"),
 ]);
 ...
 return {generatedAt,trigger,sheets:{
  ...
  'Orders':{headers:['ID','User ID','Customer','Username','Status','Request type','Primary service','All services','Options','Car ID','Car name','Plate','Vehicle','Condition','Calculated price','Final price','Currency','Emergency multiplier','Emergency surcharge','Payment status','Created at','Confirmed at','Completed at','Client deleted at','Staff deleted at'],rows:values(orders,['id','user_id','customer','username','status','request_type','service','services_json','options_json','car_id','car_name','car_plate','vehicle','condition','calculated_price','final_job_price','currency','emergency_multiplier','emergency_surcharge','payment_status','created_at','confirmed_at','completed_at','client_deleted_at','staff_deleted_at'])},
  ...
  'Client Cars':{headers:['Car ID','User ID','Customer','Username','Car name','Brand','Model','Modification','Body type','Plate','Ceramic','Owner phone','Visits','Last service','Created at','Updated at'],rows:values(cars,['id','user_id','customer','username','name','brand','model','modification','body_type','plate','has_ceramic','owner_phone','visits','last_service_at','created_at','updated_at'])},
  'Car Packages':{headers:['Car ID','User ID','Customer','Username','Car name','Plate','Main services','Options','Vehicle','Condition','Created at','Updated at'],rows:values(packages,['car_id','user_id','customer','username','car_name','plate','main_services_json','options_json','vehicle_slug','condition_slug','created_at','updated_at'])},
  'Referral Rewards':{headers:['ID','Referral ID','User ID','User','Username','Beneficiary','Reward type','Reward value','Reward currency','Reward service','Status','Created at','Used at','Used request ID'],rows:values(rewards,['id','referral_id','user_id','user','username','beneficiary','reward_type','reward_value','reward_currency','reward_service','status','created_at','used_at','used_request_id'])},
  'Reviews':{headers:['ID','User ID','User','Username','Text','Photo attached','Telegram photo file ID','Language','Status','Created at'],rows:values(reviews,['id','user_id','user','username','text','photo_attached','photo_file_id','language','status','created_at'])},
  'Suggestions':{headers:['ID','User ID','User','Username','Text','Photo attached','Telegram photo file ID','Language','Status','Created at'],rows:values(suggestions,['id','user_id','user','username','text','photo_attached','photo_file_id','language','status','created_at'])},
