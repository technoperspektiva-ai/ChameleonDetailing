...
CREATE TABLE IF NOT EXISTS client_cars(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,name TEXT NOT NULL,brand TEXT,model TEXT,modification TEXT,body_type TEXT,plate TEXT,has_ceramic INTEGER NOT NULL DEFAULT 0,owner_phone TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS car_packages(car_id INTEGER PRIMARY KEY,main_services_json TEXT NOT NULL DEFAULT '[]',options_json TEXT NOT NULL DEFAULT '[]',vehicle_slug TEXT,condition_slug TEXT,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS referral_rewards(id INTEGER PRIMARY KEY AUTOINCREMENT,referral_id INTEGER NOT NULL,user_id INTEGER NOT NULL,beneficiary TEXT NOT NULL CHECK(beneficiary IN ('REFERRER','FRIEND')),reward_type TEXT NOT NULL,reward_value REAL,reward_currency TEXT,reward_service_id INTEGER,status TEXT NOT NULL DEFAULT 'AVAILABLE',created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,used_at TEXT,used_request_id INTEGER);
...
`);
 ...
 await safeAlter(env,"ALTER TABLE service_requests ADD COLUMN car_plate TEXT");
 await safeAlter(env,"ALTER TABLE calculator_sessions ADD COLUMN services_json TEXT");
...
async function seed(env:Env){
 ...
 const serviceRows=(await env.DB.prepare("SELECT id,slug FROM services WHERE archived=0").all<any>()).results||[];
 for(const sr of serviceRows){
  const requireCondition=String(sr.slug)==='exterior-detailing'?0:1;
 ...
 for(const slug of ['anti-rain','body-bitumen-removal','body-metal-fallout-removal','hard-wax','liquid-wax','wheel-bitumen-removal','wheel-metal-fallout-removal','ceramic-spray','pet-hair','odor','carpet-ceramic','fabric-ceramic','leather-cleaning','leather-protection','seat-cleaning-1','seat-cleaning-2','seat-cleaning-4','plastic-restoration'])await linkOption('full-detailing',slug,10);
 const detailExtrasInit=await env.DB.prepare("SELECT value FROM settings WHERE key='detail_extras_v120_initialized'").first<any>();
 if(!detailExtrasInit){
  await env.DB.prepare("UPDATE service_options SET enabled=1,updated_at=CURRENT_TIMESTAMP WHERE slug IN ('anti-rain','body-bitumen-removal','body-metal-fallout-removal','hard-wax','liquid-wax','wheel-bitumen-removal','wheel-metal-fallout-removal','carpet-ceramic','fabric-ceramic','leather-cleaning','leather-protection','seat-cleaning-1','seat-cleaning-2','seat-cleaning-4','plastic-restoration')").run();
  await env.DB.prepare("INSERT OR REPLACE INTO settings(key,value,updated_at) VALUES('detail_extras_v120_initialized','1',CURRENT_TIMESTAMP)").run();
 }
