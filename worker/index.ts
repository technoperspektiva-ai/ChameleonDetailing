  ...
  try{
   ...
   if(url.pathname==='/api/services'){
    ...
    for(const item of list){const baseCurrency=String(item.currency||currency||'PLN');const eff=await serviceDisplayPrice(env,Number(item.id),Number(item.basePrice||0),baseCurrency,tier);if(eff.mode!=='STANDARD'){(item as any).standardBasePrice=item.basePrice;(item as any).basePrice=Math.round(eff.price*100)/100;(item as any).vipPricingMode=eff.mode;(item as any).clientTier=tier;(item as any).promotion=eff.promotion||null}
     if(env.DB){const req=await env.DB.prepare('SELECT require_condition,require_vehicle,allow_options,allow_multiple_options FROM service_requirements WHERE service_id=?').bind(Number(item.id)).first<any>().catch(()=>null);(item as any).requirements=req?{requireCondition:Number(req.require_condition)!==0,requireVehicle:Number(req.require_vehicle)!==0,allowOptions:Number(req.allow_options)!==0,allowMultipleOptions:Number(req.allow_multiple_options)!==0}:{requireCondition:String(item.slug)!=='exterior-detailing',requireVehicle:true,allowOptions:true,allowMultipleOptions:true}}
    }
   ...
   if(url.pathname==='/api/cars'&&request.method==='GET'){
    const u=await auth(env,url.searchParams.get('initData')||'');if(u.demo)return json({cars:[]});await ensureDb(env);
    const r=await env.DB!.prepare(`SELECT c.*,p.main_services_json,p.options_json,p.vehicle_slug package_vehicle,p.condition_slug package_condition,
      (SELECT MAX(completed_at) FROM service_requests sr WHERE sr.car_id=c.id AND sr.status='COMPLETED' AND sr.staff_deleted_at IS NULL) last_service_at,
      (SELECT COUNT(*) FROM service_requests sr WHERE sr.car_id=c.id AND sr.status='COMPLETED' AND sr.staff_deleted_at IS NULL) visits
      FROM client_cars c LEFT JOIN car_packages p ON p.car_id=c.id WHERE c.user_id=? ORDER BY c.updated_at DESC,c.id DESC`).bind(u.id).all<any>();
    return json({cars:(r.results||[]).map((x:any)=>({id:x.id,name:x.name,brand:x.brand||'',model:x.model||'',modification:x.modification||'',bodyType:x.body_type||'',plate:x.plate||'',hasCeramic:Number(x.has_ceramic||0)===1,ownerPhone:x.owner_phone||'',package:x.main_services_json?{mainServices:JSON.parse(x.main_services_json||'[]'),options:JSON.parse(x.options_json||'[]'),vehicle:x.package_vehicle||'',condition:x.package_condition||''}:null,lastServiceAt:x.last_service_at||null,visits:Number(x.visits||0)}))});
   }
   if(url.pathname==='/api/cars'&&request.method==='POST'){
    const b=await read(request),u=await auth(env,b.initData||'');if(u.demo)return json({ok:true,id:'DEMO'});await ensureDb(env);const name=String(b.name||'').trim().slice(0,80);if(!name)return json({error:'Car name is required'},400);
    const res=await env.DB!.prepare(`INSERT INTO client_cars(user_id,name,brand,model,modification,body_type,plate,has_ceramic,owner_phone) VALUES(?,?,?,?,?,?,?,?,?)`).bind(u.id,name,String(b.brand||'').trim().slice(0,60)||null,String(b.model||'').trim().slice(0,60)||null,String(b.modification||'').trim().slice(0,80)||null,String(b.bodyType||'').trim().slice(0,60)||null,String(b.plate||'').trim().slice(0,32)||null,b.hasCeramic?1:0,String(b.ownerPhone||'').trim().slice(0,40)||null).run();await event(env,u.id,'client_car_created',{carId:res.meta.last_row_id});return json({ok:true,id:res.meta.last_row_id});
   }
   const carMatch=url.pathname.match(/^\/api\/cars\/(\d+)$/);
   if(carMatch&&request.method==='PUT'){
    const b=await read(request),u=await auth(env,b.initData||'');if(u.demo)return json({ok:true});await ensureDb(env);const id=Number(carMatch[1]),old=await env.DB!.prepare('SELECT * FROM client_cars WHERE id=? AND user_id=?').bind(id,u.id).first<any>();if(!old)return json({error:'Car not found'},404);
    const name=String(b.name??old.name).trim().slice(0,80);if(!name)return json({error:'Car name is required'},400);await env.DB!.prepare(`UPDATE client_cars SET name=?,brand=?,model=?,modification=?,body_type=?,plate=?,has_ceramic=?,owner_phone=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?`).bind(name,String(b.brand??old.brand??'').trim().slice(0,60)||null,String(b.model??old.model??'').trim().slice(0,60)||null,String(b.modification??old.modification??'').trim().slice(0,80)||null,String(b.bodyType??old.body_type??'').trim().slice(0,60)||null,String(b.plate??old.plate??'').trim().slice(0,32)||null,b.hasCeramic===undefined?Number(old.has_ceramic||0):(b.hasCeramic?1:0),String(b.ownerPhone??old.owner_phone??'').trim().slice(0,40)||null,id,u.id).run();await event(env,u.id,'client_car_updated',{carId:id});return json({ok:true,id});
   }
   if(carMatch&&request.method==='DELETE'){
    const b=await read(request),u=await auth(env,b.initData||'');if(u.demo)return json({ok:true});await ensureDb(env);const id=Number(carMatch[1]);const old=await env.DB!.prepare('SELECT id FROM client_cars WHERE id=? AND user_id=?').bind(id,u.id).first<any>();if(!old)return json({error:'Car not found'},404);await env.DB!.prepare('DELETE FROM car_packages WHERE car_id=?').bind(id).run();await env.DB!.prepare('DELETE FROM client_cars WHERE id=? AND user_id=?').bind(id,u.id).run();await event(env,u.id,'client_car_deleted',{carId:id});return json({ok:true,id});
   }
   const packageMatch=url.pathname.match(/^\/api\/cars\/(\d+)\/package$/);
   if(packageMatch&&request.method==='PUT'){
    const b=await read(request),u=await auth(env,b.initData||'');if(u.demo)return json({ok:true});await ensureDb(env);const id=Number(packageMatch[1]);const car=await env.DB!.prepare('SELECT id FROM client_cars WHERE id=? AND user_id=?').bind(id,u.id).first<any>();if(!car)return json({error:'Car not found'},404);const services=Array.isArray(b.services)?b.services.map(String).filter(Boolean):[],options=Array.isArray(b.options)?b.options.map(String).filter(Boolean):[];if(!services.length)return json({error:'Choose at least one service'},400);await env.DB!.prepare(`INSERT INTO car_packages(car_id,main_services_json,options_json,vehicle_slug,condition_slug) VALUES(?,?,?,?,?) ON CONFLICT(car_id) DO UPDATE SET main_services_json=excluded.main_services_json,options_json=excluded.options_json,vehicle_slug=excluded.vehicle_slug,condition_slug=excluded.condition_slug,updated_at=CURRENT_TIMESTAMP`).bind(id,JSON.stringify([...new Set(services)]),JSON.stringify([...new Set(options)]),String(b.vehicle||'')||null,String(b.condition||'')||null).run();await event(env,u.id,'car_package_saved',{carId:id,services});return json({ok:true,id});
   }
   ...
   if(url.pathname==='/api/calculator/quote'&&request.method==='POST'){
    ...
    if(env.DB&&!u.demo){const r=await env.DB.prepare(`INSERT INTO calculator_sessions(user_id,services_json,base_price_snapshot,vehicle_multiplier_snapshot,condition_multiplier_snapshot,options_total_snapshot,discount_snapshot,calculated_price,currency,fx_rate,fx_provider,fx_timestamp,promotion_id,personal_discount_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(u.id,JSON.stringify(q.services||[q.service]),q.basePrice,q.vehicleMultiplier,q.conditionMultiplier,q.optionsTotal,q.discount,q.finalPrice,q.currency,q.fxRate,q.fxProvider,q.fxTimestamp,q.promotion?.id||null,q.personalDiscount?.id||null).run();(q as any).calculationId=r.meta.last_row_id;await event(env,u.id,'calculator_completed',{services:q.services||[q.service],currency:q.currency})}
   ...
   if(url.pathname==='/api/orders/request'&&request.method==='POST'){
    ...
    let car:any=null;const carId=Number(b.carId||0);if(carId){car=await env.DB!.prepare('SELECT id,name,plate FROM client_cars WHERE id=? AND user_id=?').bind(carId,u.id).first<any>();if(!car)return json({error:'Car not found'},404)}
    const res=await env.DB!.prepare(`INSERT INTO service_requests(user_id,service_slug,services_json,vehicle_slug,condition_slug,options_json,request_type,is_deferred,scheduled_for,base_price_snapshot,options_total_snapshot,discount_snapshot,calculated_price,currency,emergency_multiplier,emergency_surcharge,promotion_id,personal_discount_id,car_id,car_name,car_plate) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(u.id,(q.services||[q.service])[0]||null,JSON.stringify(q.services||[q.service]),b.vehicle||null,q.requiresCondition?(b.condition||null):null,JSON.stringify(q.options||b.options||[]),requestType,requestType==='DEFERRED'?1:0,requestType==='DEFERRED'?state.schedule.nextWorkingAt:null,q.basePrice,q.optionsTotal,q.discount,q.finalPrice,q.currency,requestType==='EMERGENCY'?em:null,q.emergencySurcharge||0,q.promotion?.id||null,q.personalDiscount?.id||null,car?.id||null,car?.name||null,car?.plate||null).run();
    await event(env,u.id,'service_request_created',{id:res.meta.last_row_id,type:requestType,price:q.finalPrice,currency:q.currency,services:q.services||[q.service],carId:car?.id||null});
    await env.DB!.prepare("UPDATE referrals SET request_created_at=COALESCE(request_created_at,CURRENT_TIMESTAMP) WHERE referred_user_id=?").bind(u.id).run().catch(()=>{});
    ...
    if(q.referralReward?.id){await env.DB!.prepare("UPDATE referral_rewards SET status='USED',used_at=CURRENT_TIMESTAMP,used_request_id=? WHERE id=? AND user_id=? AND status='AVAILABLE'").bind(res.meta.last_row_id,q.referralReward.id,u.id).run();await event(env,u.id,'referral_reward_used',{rewardId:q.referralReward.id,requestId:res.meta.last_row_id,type:q.referralReward.type});}
