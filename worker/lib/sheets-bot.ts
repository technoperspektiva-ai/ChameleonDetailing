...
async function render(env:SheetsEnv,msg:TgMessage,from:TgFrom,notice=''){
 ...
 const rows:any[]=[
  [{text:l3(locale,'🔄 Синхронізувати зараз','🔄 Synchronizuj teraz','🔄 Sync now'),callback_data:'sheets:sync'},{text:l3(locale,'🩺 Перевірити','🩺 Sprawdź','🩺 Check'),callback_data:'sheets:health'}],
  [{text:`${state.interval==='OFF'?'✅ ':''}OFF`,callback_data:'sheets:interval:OFF'},{text:`${state.interval==='1h'?'✅ ':''}1h`,callback_data:'sheets:interval:1h'},{text:`${state.interval==='6h'?'✅ ':''}6h`,callback_data:'sheets:interval:6h'},{text:`${state.interval==='24h'?'✅ ':''}24h`,callback_data:'sheets:interval:24h'}],
 ];
...
export async function handleGoogleSheetsTelegramUpdate(env:SheetsEnv,update:TgUpdate){
 ...
 if(data==='sheets:health'){
  try{const health=await checkGoogleSheetsHealth(env);await render(env,msg,from,l3(locale,`✅ Apps Script відповідає. Таблиця: <b>${esc(health.title||'OK')}</b>.`,`✅ Apps Script odpowiada. Arkusz: <b>${esc(health.title||'OK')}</b>.`,`✅ Apps Script is responding. Sheet: <b>${esc(health.title||'OK')}</b>.`))}catch(e:any){await render(env,msg,from,`⚠️ ${esc(e?.message||e)}`)}return true;
 }
 const interval=data.match(/^sheets:interval:(OFF|1h|6h|24h)$/);if(interval){await setGoogleSheetsInterval(env,interval[1] as SheetsInterval);await render(env,msg,from,l3(locale,'✅ Період синхронізації змінено.','✅ Zmieniono okres synchronizacji.','✅ Sync interval updated.'));return true}
 await render(env,msg,from);return true;
}
