...
export async function quote(env:Env,input:any,tier='STANDARD',emergencyMultiplier=1){
 ...
 return {service:s.slug,standardBasePrice:round(standardBase*rate),basePrice:round((subtotal-optionsBase)*rate/(vm*cm||1)),vehicleMultiplier:vm,conditionMultiplier:cm,optionsTotal:round(optionsBase*rate),discount:round(discount*rate),vipPricing:{tier,mode:pricingMode,rule:appliedRule?{percentDiscount:appliedRule.percent_discount,multiplier:appliedRule.multiplier,fixedPrice:appliedRule.fixed_price,currency:appliedRule.currency}:null},emergencyMultiplier,emergencySurcharge:round(emergencySurcharge*rate),finalPrice:round((subtotal+emergencySurcharge)*rate),currency:target,fxRate:rate,fxProvider:'FALLBACK_STATIC',fxTimestamp:new Date().toISOString(),options:opts};
}
