...
export async function buildReport(env:Env,type:ReportType,days=30,locale:ReportLocale='en'){
 ...
 if(type==='business')addSheet(wb,'Client Cars',[{header:'ID',key:'id'},{header:'User ID',key:'user_id'},{header:'User',key:'first_name'},{header:'Username',key:'username'},{header:'Name',key:'name'},{header:'Brand',key:'brand'},{header:'Model',key:'model'},{header:'Modification',key:'modification'},{header:'Body type',key:'body_type'},{header:'Plate',key:'plate'},{header:'Ceramic',key:'has_ceramic'},{header:'Owner phone',key:'owner_phone'},{header:'Created',key:'created_at'},{header:'Last visit',key:'last_service_at'}],await rows(env,`SELECT c.*,u.first_name,u.username,(SELECT MAX(completed_at) FROM service_requests r WHERE r.car_id=c.id AND r.status='COMPLETED' AND r.staff_deleted_at IS NULL) last_service_at FROM client_cars c LEFT JOIN users u ON u.id=c.user_id ORDER BY c.id DESC`));
 if(type==='business')addSheet(wb,'Car Packages',[{header:'Car ID',key:'car_id'},{header:'User',key:'first_name'},{header:'Username',key:'username'},{header:'Car name',key:'car_name'},{header:'Plate',key:'plate'},{header:'Main services',key:'main_services_json'},{header:'Options',key:'options_json'},{header:'Vehicle',key:'vehicle_slug'},{header:'Condition',key:'condition_slug'},{header:'Created',key:'created_at'}],await rows(env,`SELECT p.*,c.user_id,c.name car_name,c.plate,u.first_name,u.username FROM car_packages p JOIN client_cars c ON c.id=p.car_id LEFT JOIN users u ON u.id=c.user_id ORDER BY p.updated_at DESC`));
 if(type==='referrals'||type==='business')addSheet(wb,'Referral Rewards',[{header:'ID',key:'id'},{header:'Referral ID',key:'referral_id'},{header:'User ID',key:'user_id'},{header:'User',key:'first_name'},{header:'Username',key:'username'},{header:'Beneficiary',key:'beneficiary'},{header:'Reward type',key:'reward_type'},{header:'Value',key:'reward_value'},{header:'Currency',key:'reward_currency'},{header:'Service',key:'reward_service'},{header:'Status',key:'status'},{header:'Created',key:'created_at'},{header:'Used at',key:'used_at'},{header:'Order ID',key:'used_request_id'}],await rows(env,`SELECT rr.*,u.first_name,u.username,s.slug reward_service FROM referral_rewards rr LEFT JOIN users u ON u.id=rr.user_id LEFT JOIN services s ON s.id=rr.reward_service_id WHERE ${daysClause(days,'rr.')} ORDER BY rr.id DESC`));
